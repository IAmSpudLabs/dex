...
const main = async () => {
  ...
  const r5 = await knex<User>("users").select(
  ...
  // $ExpectType (Pick<User, "name"> & { identifier: number; })[]
  const r9 = await knex.select(knex.ref("id").as("identifier"), "name").from<
    User
  >("users");
  ...
  // $ExpectType { count: number; }
  await knex.first().countDistinct<{ count: number }>({ count: "*" }).from(
    "foo",
  );
